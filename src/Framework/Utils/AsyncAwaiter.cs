...
namespace Framework.Utils
{
    ...
    /// </summary>
    public class AsyncAwaiter : IAsyncAwaiter
    {
        ...
        /// <returns></returns>
        public Task<T> AwaitResultAsync<T>(string key, Func<Task<T>> task, int maxAccessCount = 1)
        {
            return (Task<T>)AwaitAsync(key, task, maxAccessCount);
        }
        ...
        /// <param name="maxAccessCount">If this is the first call, sets the maximum number of tasks that can access this task before it waiting</param>
        /// <returns></returns>
        public async Task AwaitAsync(string key, Func<Task> task, int maxAccessCount = 1)
        {
